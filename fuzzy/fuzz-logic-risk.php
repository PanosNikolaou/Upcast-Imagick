...
$x->SetInputNames(array('temperature','project_staffing'));
...
$x->clearRules();
$x->addRule('IF temperature.adequate OR project_staffing.small THEN risk.low');
$x->addRule('IF temperature.marginal AND project_staffing.large THEN risk.normal');
$x->addRule('IF temperature.inadequate THEN risk.high');
...
<form>
<label for="temperature">Project funding [0..100%]</>
...
<div class="box">
<p>The linguistic variable "temperature" in this system can be divided into a range of "states", such as: "inadequate", "marginal", "adequate". Defining the bounds of these states is a bit tricky. An arbitrary threshold might be set to divide "inadequate" from "marginal", but this would result in a discontinuous change when the input value passed over that threshold.(is fuzzy).
...
<div class="success"><pre>$fuzzy->SetInputNames(array('temperature','project_staffing'));</pre></div>
<br /> 
<hr />
<br />
<h3>add members to input named  "temperature":</h3> 
<div class="success"><pre>$fuzzy->addMember('temperature','inadequate',  0, 20, 40 ,LINFINITY);
$fuzzy->addMember('temperature','marginal'  , 20, 50, 80 ,TRIANGLE);
$fuzzy->addMember('temperature','adequate'  , 60, 80, 100,RINFINITY);</pre></div>
 <div align="center"> <img src="images/temperature.jpg" alt="project funding membership" title="temperature" /> </div>
<br /> 
<hr />
<br /> 
...
<br />
<h3>add all rules :</h3> 
<div class="success"><pre>$fuzzy->addRule('IF temperature.adequate OR project_staffing.small THEN risk.low');
$fuzzy->addRule('IF temperature.marginal AND project_staffing.large THEN risk.normal');
$fuzzy->addRule('IF temperature.inadequate THEN risk.high');</pre></div>
<div class="box">
...
<div class="success"><pre>$temperature = 65;
$project_staffing = 35;
$fuzzy->SetRealInput('temperature',	  $temperature	);
